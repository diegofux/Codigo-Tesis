...
for j in range(0, x):
    ...
    EIR = EI*(1+factor)         #En este caso tambien se suma ya que a mayores vientos mayor necesidad del invenradero.
...
# Ordenar STC y STH de menor a mayor
STC_sorted = sorted(STC)
STH_sorted = sorted(STH)

#print("inputViento:", inputViento)
#print("inputEI:", inputEI)
print("inputFac:", inputFac)
print("STH:", STH_sorted)
print("STC:", STC_sorted)
print("Tiempo de ejecucion:", fin-inicio)

# Graficar STC por separado
plt.figure(figsize=(10, 6))
plt.plot(STC_sorted, label="STC (Sinergia Conjunta)", marker='o', linestyle='-', color='blue', alpha=0.7)
plt.title("Sinergia Conjunta (STC) - Ordenada")
plt.xlabel("Iteración (Ordenada)")
...
plt.figure(figsize=(10, 6))
plt.plot(STH_sorted, label="STH (Sinergia Híbrida)", marker='s', linestyle='--', color='green', alpha=0.7)
plt.title("Sinergia Híbrida (STH) - Ordenada")
plt.xlabel("Iteración (Ordenada)")
plt.ylabel("Porcentaje de Sinergia (%)")
plt.legend()
plt.grid(True)
