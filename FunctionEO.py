...
def SinergiasEO (EI,Rad):
    ...
    for a in range(len(Cal)):
        ...
        prob += PE*CE + sum([PB[i]*CB[i] for i in Baterias]) + PPV*CEO + Pcal*Ccal
        ...
        Z=PE*CE + sum([PB[i]*CB[i] for i in Baterias]) + PPV*CEO + Pcal*Ccal
    ...
    #Cada sinergia es el porcentaje de ahorro vs el caso base, es decir una sinergia de 10% significa una reduccion del 10% vs el caso Base
    SinergiaC=(Base-Conjunto)*100/Base
    SinergiaH=(Base-Hibrido)*100/Base
    #print("SinergiaH:", SinergiaH)
    #print("SinergiaC:", SinergiaC)
    return SinergiaH,SinergiaC
