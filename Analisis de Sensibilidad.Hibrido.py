...
Eh= LpVariable("Eh",5, None) #en el caso conjunto el minimo es 0, tiene que estar en kW.
...
for i in Baterias:
    for j in Hora:    
        if j==len(Hora)-1:        #Restricción para la hora final                                                                    
            prob += B_init[i] == B[i][j] + EBC[i][j]*eta_C - EBD[i][j]*(1/eta_D)  
        elif j==0:#Restricción para la hora inicial
            prob += B[i][j+1] == B_init[i] + EBC[i][j]*eta_C - EBD[i][j]*(1/eta_D)
            prob += B[i][j] == B_init[i] 
        else: # Restricción para el resto de horas
